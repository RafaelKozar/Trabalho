...
var pacientesVazio = [{ nome : "nao" }];
var robosVazio = [{ nome : "nao" }];
var usersVazio = [{ nome : "nao" }];
...
module.exports = function (app, passport) {
    ...
    app.get('/cadastraruser', isLoggedIn, function (req, res) {
        var pacientes = pacienteDAO.listarPacientes(function (paciente) {
            if (user)                
                res.render('cadastraruser.ejs', { dados : pacientes});
            else                 
                res.render('cadastraruser.ejs', { dados : pacientesVazio, message : req.flash('message', "") });            
        });
    });
    ...
    app.get('/cadastrarpaciente', isLoggedIn, function (req, res) {
        var robos = roboDAO.listarRobos(function (robos) {
            if (robos)
                //res.redirect('listarpacientes.ejs');
    ...
    app.get('/listarpacientes', isLoggedIn, function (req, res) {
        var pacientes = pacienteDAO.listarPacientes(function (pacientes) {
            if(pacientes)
                res.render('listarpacientes.ejs', { dados : pacientes })
            else
                res.render('listarpacientes.ejs', { dados : pacientesVazio })
        });
    });
